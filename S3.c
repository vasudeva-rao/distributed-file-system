 ...
 * Handles text file operations in the distributed file system
 * This server is responsible for:
 * - Storing and managing TXT files
 * - Processing upload/download requests
 * - Creating tar archives of text files
...
char* map_s1_to_s3_path(const char* path, char* mapped, size_t size);
...
int main() {
    ...
    if ((server_fd = init_server(S3_PORT)) < 0) {
        fprintf(stderr, "Failed to initialize server\n");
        return 1;
    }
    
    printf("TXT Server (S3) started on port %d\n", S3_PORT);
...
void process_client(int client_fd) {
    ...
    while (1) {
        ...
        if (strcmp(cmd.cmd_type, CMD_UPLOADF) == 0) {
            if (!map_s1_to_s3_path(cmd.arg2, mapped_path, sizeof(mapped_path))) {
            ...
            if (!map_s1_to_s3_path(cmd.arg1, mapped_path, sizeof(mapped_path))) {
                send(client_fd, "Error: Invalid path", 18, 0);
                continue;
            }
            ...
            if (strcmp(filetype, ".txt") != 0) {
                send(client_fd, "Error: S3 only handles .txt files", 31, 0);
                continue;
            }
            
            // Create temporary tar file
            char temp_path[MAX_PATH];
            snprintf(temp_path, sizeof(temp_path), "/tmp/txt_XXXXXX");
            ...
            snprintf(cmd, sizeof(cmd), 
                "cd %s && find S3 -name '*.txt' -print0 | tar czf %s --null -T -",
                home, temp_path);
            ...
            snprintf(find_cmd, sizeof(find_cmd),
                "find %s -type f -name '*.txt' -printf '%%P\\n' 2>/dev/null | sort",
                mapped_path);
            ...
            if (bytes == 0) {
                ...
                send(client_fd, "", 0, 0);
                printf("Debug: No .txt files found in %s\n", mapped_path);
            } else {
                send(client_fd, result, strlen(result), 0);
                printf("Debug: Found and sent list of .txt files\n");
            }
        }
    }
}
...
/* Function to validate and map S1 path to S3 path
 * Validates and maps an S1 path to an S3 path
...
char* map_s1_to_s3_path(const char* path, char* mapped, size_t size) {
    ...
    // Check if path starts with ~/S1 or ~/S3
    if (strncmp(path, "~/S1", 4) != 0 && strncmp(path, "~/S3", 4) != 0) {
        return NULL;
    }
    ...
    // Create the new path with S3
    if (snprintf(mapped, size, "%s/S3%s", home, remainder) >= (int)size) {
