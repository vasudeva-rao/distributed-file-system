/**
 * S4 Server Implementation
 * Handles ZIP file operations in the distributed file system
 * This server is responsible for:
 * - Storing and managing ZIP files
 * - Processing upload/download requests
 * - Creating tar archives of ZIP files
...
char* map_s1_to_s4_path(const char* path, char* mapped, size_t size);
...
int main() {
    ...
    if ((server_fd = init_server(S4_PORT)) < 0) {
        fprintf(stderr, "Failed to initialize server\n");
        return 1;
    }
    
    printf("ZIP Server (S4) started on port %d\n", S4_PORT);
...
void process_client(int client_fd) {
    ...
    while (1) {
        ...
        if (strcmp(cmd.cmd_type, CMD_UPLOADF) == 0) {
            if (!map_s1_to_s4_path(cmd.arg2, mapped_path, sizeof(mapped_path))) {
            ...
            if (!map_s1_to_s4_path(cmd.arg1, mapped_path, sizeof(mapped_path))) {
                send(client_fd, "Error: Invalid path", 18, 0);
                continue;
            }
            ...
            if (!home) {
                send(client_fd, "Error: Cannot determine home directory", 36, 0);
                continue;
            }
            
            char temp_file[] = "/tmp/zip_files.tar";
            char tar_cmd[MAX_BUFFER];
            snprintf(tar_cmd, sizeof(tar_cmd), 
                "cd %s && find S4 -name '*.zip' -print0 | tar czf %s --null -T -",
                home, temp_file);
            
            system(tar_cmd);
            send_file(client_fd, temp_file);
            remove(temp_file);
            ...
            // Use -maxdepth 1 to only list files in the current directory
            snprintf(find_cmd, sizeof(find_cmd), 
                "find %s -type f -name '*.zip' -printf '%%P\\n' 2>/dev/null | sort",
                mapped_path);
            ...
            if (bytes == 0) {
                ...
                send(client_fd, "", 0, 0);
                printf("Debug: No .zip files found in %s\n", mapped_path);
            } else {
                send(client_fd, result, strlen(result), 0);
                printf("Debug: Found and sent list of .zip files\n");
            }
        }
    }
}
...
int init_server(int port) {
    ...
    int opt = 1;
    if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0 || // Allow reuse of the address
        bind(server_fd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0 || // Bind the socket to the address
        listen(server_fd, 10) < 0) { // Listen for incoming connections
        close(server_fd);
        return ERR_SOCKET;
    }
...
/* Function to validate and map S1 path to S4 path
 * Validates and maps an S1 path to an S4 path
...
char* map_s1_to_s4_path(const char* path, char* mapped, size_t size) {
    ...
    // Check if path starts with ~/S1 or ~/S4
    if (strncmp(path, "~/S1", 4) != 0 && strncmp(path, "~/S4", 4) != 0) {
        return NULL;
    }
    ...
    // Create the new path with S4
    if (snprintf(mapped, size, "%s/S4%s", home, remainder) >= (int)size) {
